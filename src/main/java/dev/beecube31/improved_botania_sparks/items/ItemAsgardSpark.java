...
public class ItemAsgardSpark extends ManaSparkItem {
    public ItemAsgardSpark(Properties builder) {
        super(builder);
    }
    ...
    public void appendHoverText(@NotNull ItemStack stack, Level world, @NotNull List<Component> tooltip, @NotNull TooltipFlag flags) {
        tooltip.add(Component.translatable("ibs.transfer_rate.formatted", IBSConfig.COMMON.asgardSparkTransferRate.get())
                .withStyle(ChatFormatting.ITALIC, ChatFormatting.GRAY));

        tooltip.add(Component.translatable("ibs.work_range.formatted", IBSConfig.COMMON.asgardSparkWorkRange.get())
                .withStyle(ChatFormatting.ITALIC, ChatFormatting.GRAY));
    }
        ...
        if (attach != null) {
            if (attach.canAttachSpark(stack) && attach.getAttachedSpark() == null) {
                if (!world.isClientSide) {
                    stack.shrink(1);
                    EntityImprovedSpark spark = new EntityImprovedSpark(IBSEntities.ASGARD_SPARK, world, IBSConfig.COMMON.asgardSparkTransferRate, IBSConfig.COMMON.asgardSparkWorkRange);
