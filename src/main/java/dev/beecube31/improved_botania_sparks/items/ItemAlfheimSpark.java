...
public class ItemAlfheimSpark extends ManaSparkItem {
    public ItemAlfheimSpark(Properties builder) {
        super(builder);
    }
    ...
    public void appendHoverText(@NotNull ItemStack stack, Level world, @NotNull List<Component> tooltip, @NotNull TooltipFlag flags) {
        tooltip.add(Component.translatable("ibs.transfer_rate.formatted", IBSConfig.COMMON.alfheimSparkTransferRate.get())
                .withStyle(ChatFormatting.ITALIC, ChatFormatting.GRAY));

        tooltip.add(Component.translatable("ibs.work_range.formatted", IBSConfig.COMMON.alfheimSparkWorkRange.get())
                .withStyle(ChatFormatting.ITALIC, ChatFormatting.GRAY));
    }
        ...
        if (attach != null) {
            if (attach.canAttachSpark(stack) && attach.getAttachedSpark() == null) {
                if (!world.isClientSide) {
                    stack.shrink(1);
                    EntityImprovedSpark spark = new EntityImprovedSpark(IBSEntities.ALFHEIM_SPARK, world, IBSConfig.COMMON.alfheimSparkTransferRate, IBSConfig.COMMON.alfheimSparkWorkRange);
